...
//! Vertical motions (`move_up` / `move_down`) take a caller-owned
//! `sticky_col` (vim's `curswant`) so bouncing through a shorter
//! row doesn't drag the cursor back to col 0. Word motions
//! (`move_word_*`) take an `iskeyword` spec so the host can change
//! it without re-publishing it onto the buffer. Both lived as
//! buffer fields up to 0.0.27; 0.0.28 hoisted them onto `Editor`.
...
impl Buffer {
    ...
    pub fn move_up(&mut self, count: usize, sticky_col: &mut Option<usize>) {
        self.move_vertical(-(count.max(1) as isize), sticky_col);
    }

    /// `j` — `count` rows down. See [`Buffer::move_up`] for sticky
    /// ownership.
    pub fn move_down(&mut self, count: usize, sticky_col: &mut Option<usize>) {
        self.move_vertical(count.max(1) as isize, sticky_col);
    }
    ...
    /// segments. `sticky_col` ownership matches [`Buffer::move_up`].
    pub fn move_screen_up(&mut self, count: usize, sticky_col: &mut Option<usize>) {
        self.move_screen_vertical(-(count.max(1) as isize), sticky_col);
    }

    /// `gj` — `count` visual rows down. See [`Buffer::move_screen_up`].
    pub fn move_screen_down(&mut self, count: usize, sticky_col: &mut Option<usize>) {
        self.move_screen_vertical(count.max(1) as isize, sticky_col);
    }
    ...
    pub fn move_word_fwd(&mut self, big: bool, count: usize, iskeyword: &str) {
        for _ in 0..count.max(1) {
            let from = self.cursor();
            if let Some(next) = next_word_start(self, from, big, iskeyword) {
    ...
    pub fn move_word_back(&mut self, big: bool, count: usize, iskeyword: &str) {
        for _ in 0..count.max(1) {
            let from = self.cursor();
            if let Some(prev) = prev_word_start(self, from, big, iskeyword) {
    ...
    pub fn move_word_end(&mut self, big: bool, count: usize, iskeyword: &str) {
        for _ in 0..count.max(1) {
            let from = self.cursor();
            if let Some(end) = next_word_end(self, from, big, iskeyword) {
                self.set_cursor(end);
            } else {
                break;
            }
    ...
    pub fn move_word_end_back(&mut self, big: bool, count: usize, iskeyword: &str) {
        for _ in 0..count.max(1) {
            let from = self.cursor();
            match prev_word_end(self, from, big, iskeyword) {
    ...
    fn move_screen_vertical(&mut self, delta: isize, sticky_col: &mut Option<usize>) {
        let v = self.viewport();
        if matches!(v.wrap, crate::Wrap::None) || v.text_width == 0 {
            self.move_vertical(delta, sticky_col);
            return;
        }
        ...
        *sticky_col = Some(self.cursor().col);
    }
    ...
    fn move_vertical(&mut self, delta: isize, sticky_col: &mut Option<usize>) {
        let cursor = self.cursor();
...
mod tests {
    ...
    fn move_word_back_lands_on_word_start() {
        ...
        b.move_word_back(false, 1, ISK);
        assert_eq!(at(&b), Position::new(0, 8));
        b.move_word_back(false, 2, ISK);
        assert_eq!(at(&b), Position::new(0, 0));
    }
    ...
    fn move_word_end_back_lands_on_prev_word_end() {
        ...
        b.move_word_end_back(false, 1, ISK);
        assert_eq!(at(&b), Position::new(0, 6));
        b.move_word_end_back(false, 1, ISK);
        assert_eq!(at(&b), Position::new(0, 2));
    }
    ...
    fn move_word_end_back_big_skips_punct() {
        ...
        b.set_cursor(Position::new(0, 10));
        b.move_word_end_back(true, 1, ISK);
        assert_eq!(at(&b), Position::new(0, 6));
    }
    ...
    fn screen_down_falls_back_to_move_down_when_wrap_off() {
        let mut b = Buffer::from_str("a\nb\nc");
        let mut sticky = None;
        b.move_screen_down(1, &mut sticky);
        assert_eq!(at(&b), Position::new(1, 0));
        b.move_screen_down(1, &mut sticky);
        assert_eq!(at(&b), Position::new(2, 0));
    }
    ...
    fn screen_down_count_compounds() {
        ...
        b.set_cursor(Position::new(0, 0));
        let mut sticky = None;
        b.move_screen_down(2, &mut sticky);
        assert_eq!(at(&b), Position::new(0, 8));
    }
}
